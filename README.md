...
- **ניהול ייצור של מאפים** – כולל יצירת טבלה רלוונטית, טריגרים, פונקציות, פרוצדורות ותוכנית ראשית.
...
# 🧁 מערכת GUI לניהול מאפייה – מסדי נתונים ודוחות

אפליקציית GUI זו נבנתה בפייתון ומאפשרת ניהול מלא של נתוני מאפייה – עובדים, מוצרי מאפה, פסי ייצור, ודוחות מתקדמים. המערכת כוללת שאילתות מורכבות, תמיכה בטרנזקציות, ממשק ידידותי לפעולות CRUD ועוד.

---

## 📚 תוכן העניינים

1. [📌 מסך פתיחה – ניווט ראשי](#-מסך-פתיחה--ניווט-ראשי)
2. [👨‍🍳 ניהול עובדים – CRUD מלא](#-ניהול-עובדים--crud-מלא)
3. [🥐 ניהול מוצרי מאפה – טבלאות משולבות](#-ניהול-מוצרי-מאפה--טבלאות-משולבות)
4. [🏭 ניהול פסי ייצור – פונקציה ופרוצדורה](#-ניהול-פסי-ייצור--פונקציה-ופרוצדורה)
5. [📊 דוחות ושאילתות – SELECT, UPDATE, DELETE](#-דוחות-ושאילתות--select-update-delete-עם-תמיכה-בtransactions)

---

## 📌 מסך פתיחה – ניווט ראשי
בעת הרצת קובץ ה־Python של האפליקציה, מופיע **מסך פתיחה** שמרכז את כל הפעולות המרכזיות במערכת. במסך זה מוצגים חמישה כפתורים, כאשר כל כפתור מפנה למסך ניהול אחר:


#### 🧑‍💼 ניהול עובדים (CRUD)
- לחיצה על כפתור זה פותחת את מסך ניהול העובדים.
- במסך זה ניתן לבצע פעולות **CRUD (יצירה, קריאה, עדכון ומחיקה)** על **טבלת העובדים**.


#### 🥐 ניהול מוצרי המאפה (CRUD)
- לחיצה על כפתור זה פותחת את מסך ניהול המוצרים.
- המסך מציג טבלה מאוחדת של:
  - **מוצרי מאפה**
  - **קטגוריות**
  - **ערכים תזונתיים**
- ניתן לבצע פעולות **CRUD** מלאות על המוצרים.


#### 🏭 ניהול פסי ייצור
- כפתור זה פותח את מסך ניהול פסי הייצור.
- במסך זה מופעלות:
  - **פונקציה**
  - **פרוצדורה**
- הפונקציונליות מאפשרת להזין ולעדכן נתונים על תהליך הייצור בצורה אוטומטית.


#### 📊 שאילתות ודוחות
- כפתור זה פותח את מסך השאילתות והדוחות.
- ניתן להריץ מגוון **שאילתות** ולקבל **דוחות מותאמים** מתוך בסיס הנתונים.


#### ❌ יציאה
- כפתור ליציאה מהמערכת.


---

## 👨‍🍳 ניהול עובדים – CRUD מלא
מסך ניהול העובדים מאפשר לבצע מגוון פעולות על טבלת העובדים בבסיס הנתונים, ומחולק לשלושה אזורים עיקריים:


#### 🔍 חלק ראשון – חיפוש עובדים
- שדה **Dropdown List** לבחירת עמודת חיפוש (לפי שם, טלפון, אימייל, סניף וכו').
- מאפשר סינון מהיר של העובדים בהתאם לערך שנבחר.


#### ✏️ חלק שני – פעולות על טבלת העובדים

##### ➕ הוספת עובד
- לחיצה על כפתור "הוספת עובד" תפתח טופס להזנת פרטי העובד:
  - שם, טלפון, אימייל, תאריך לידה וכו'.
  - **יש Dropdown לבחירת סניף** מתוך רשימת סניפים קיימת.
  - **יש Dropdown לבחירת תפקיד** מתוך רשימת תפקידים.

##### 🔁 עדכון עובד
- יש לבחור עובד מהרשימה (חלק שלישי).
- לאחר הבחירה, לחיצה על כפתור "עדכון עובד" תפתח טופס עם הפרטים הקיימים.
- ניתן לערוך את הפרטים וללחוץ על:
  - **"עדכן"** – יעדכן את העובד במסד הנתונים.
  - **"ביטול"** – יסגור את הטופס מבלי לשמור שינויים.

##### 🗑️ מחיקת עובד
- לאחר בחירת עובד מהרשימה, לחיצה על "מחק עובד" תבצע:
  - תיבת **אימות** (confirmation) לפני המחיקה.
  - במקרה של אישור, העובד יימחק מבסיס הנתונים.

##### 🔄 רענון
- כפתור "רענן" מבצע:
  - רענון מלא של נתוני העובדים.
  - הסתרת כל טופסי ההוספה/עדכון.
  - הצגת רשימה עדכנית בהתאם לפעולות האחרונות.


#### 📋 חלק שלישי – רשימת עובדים

- טבלת תצוגה של כלל העובדים, מסודרת **בסדר אלפביתי**.
- מציגה את כל פרטי העובד הנדרשים.
- מתעדכנת באופן אוטומטי לאחר:
  - הוספה
  - עדכון
  - מחיקה
  - חיפוש
- תוצאות החיפוש מופיעות גם הן בטבלה זו.


---

## 🥐 ניהול מוצרי מאפה – טבלאות משולבות
מסך ניהול מוצרי המאפה מאפשר לנהל נתונים מתוך שלוש טבלאות שונות באמצעות טבלה אחת משולבת (JOIN). הטבלאות הן:

- טבלת **BakedGoods** – מוצרי המאפה
- טבלת **Categories** – קטגוריות
- טבלת **NutritionFacts** – ערכים תזונתיים

המסך מחולק לשלושה אזורים עיקריים:


#### 🔍 חלק ראשון – חיפוש מוצרים
- שדה **Dropdown List** לבחירת עמודת החיפוש.
- החיפוש נעשה לפי עמודה בטבלת JOIN (למשל לפי שם מוצר, קטגוריה, קלוריות וכו').
- מאפשר סינון מהיר של המוצרים המוצגים בטבלה.


#### ✏️ חלק שני – פעולות על המוצרים

##### ➕ הוספת מוצר אפייה
- לחיצה על כפתור "הוסף מוצר" פותחת טופס להזנת פרטי המוצר, כולל:
  - שם, תיאור, מחיר, ערכים תזונתיים ועוד.
  - יש **Dropdown לבחירת הקטגוריה** מתוך טבלת הקטגוריות.
- לאחר מילוי הנתונים ולחיצה על "הוסף", מתבצעת:
  - הוספה לטבלת **BakedGoods**
  - הוספה לטבלת **NutritionFacts**

##### 🔁 עדכון מוצר
- יש לבחור מוצר מתוך הטבלה (חלק שלישי).
- לחיצה על כפתור "עדכן מוצר" תפתח טופס עם הפרטים הקיימים.
- ניתן לשנות כל ערך בטופס, ולאחר לחיצה על "עדכן":
  - הטבלאות **BakedGoods** ו־**NutritionFacts** מתעדכנות בהתאם.
- לחיצה על "ביטול" תסגור את הטופס מבלי לשמור שינויים.

##### 🗑️ מחיקת מוצר
- יש לבחור מוצר מתוך הטבלה.
- לחיצה על "מחק מוצר" תבצע מחיקה מתוך **BakedGoods בלבד**.
  - אין צורך למחוק ידנית מטבלאות הקשורות, בזכות הגדרה של:
    - פקודת `ON DELETE CASCADE` על המפתחות הזרים.
  - הטבלאות NutritionFacts ו־Categories יתעדכנו אוטומטית בהתאם.

##### 🔄 רענון
- כפתור "רענן" מסתיר את הטפסים ומרענן את הטבלה.
- מאפשר לראות את המידע העדכני ביותר לאחר כל פעולה.


#### 📋 חלק שלישי – רשימת מוצרי מאפה

- טבלה המציגה את רשימת המוצרים המלאה.
- מבוססת על **JOIN** של שלוש הטבלאות: BakedGoods, Categories, NutritionFacts.
- מוצגת **בסדר אלפביתי** לפי שם המוצר.
- מתעדכנת אוטומטית לאחר:
  - הוספה
  - עדכון
  - מחיקה
  - חיפוש
...
💡 פעולות ההוספה והעדכון כוללות תמיד עדכון סינכרוני של:
- טבלת `BakedGoods`  
- וטבלת `NutritionFacts`



---

## 🏭 ניהול פסי ייצור – פונקציה ופרוצדורה
מערכת ניהול תהליכי ייצור חכמה הכוללת בדיקת מלאי, הרצת פרוצדורות ייצור, וניהול חומרי גלם בזמן אמת.


המסך מחולק ל-5 אזורים עיקריים:

#### 1. כותרת המערכת (חלק עליון)
![image](https://github.com/user-attachments/assets/9344a705-4ef9-414e-bfc0-82b6903438ed)

- כותרת: "הרצת ייצור למוצר מאפה"


#### 2. אזור בחירת פרמטרים (חלק מרכזי עליון)

![image](https://github.com/user-attachments/assets/bbea1595-41c6-41f0-8591-7da601fb027d)


**בחירת סניף:**
- תפריט נפתח עם כל הסניפים הפעילים
- הצגה: "מזהה - מיקום"
- טעינה אוטומטית מטבלת Branches
- עדכון דינמי של רשימת העובדים

**בחירת עובד:**
- תפריט דינמי המתעדכן לפי הסניף הנבחר
- הצגה: "מזהה - שם העובד"
- שליפה מטבלת Employee לפי branchId

**בחירת מוצר אפייה:**
- תפריט עם כל מוצרי המאפייה
- הצגה: "מזהה - שם המוצר"
- טעינה מטבלת BakedGoods

#### 3. כפתור בדיקת חומרים

![image](https://github.com/user-attachments/assets/f1d3bd32-d55f-414b-bd6e-a06fc3ad006a)


- טקסט: "↻ בדוק חומרים למוצר"
- פונקציה: הצגת סיכום חומרים נדרשים למוצר

#### 4. שדה כמות הייצור

![image](https://github.com/user-attachments/assets/7cb7d59c-e293-469f-b721-6dda711306a2)


- תווית: "כמות לייצור:"
- ואלידציה: בדיקת מספר חיובי

#### 5. כפתור הרצת הייצור (תחתון)

![image](https://github.com/user-attachments/assets/d23d3716-699f-4cf3-9803-e861332e8306)
...
#### הרצת תהליך ייצור רגיל
1. **בחירת פרמטרים:**
   - בחירת סניף מהתפריט הנפתח
   - בחירת עובד (מתעדכן אוטומטית לפי הסניף)
   - בחירת מוצר אפייה
   - הזנת כמות הייצור

2. **ואלידציה:**
   - בדיקה שכל השדות מולאו
   - הדגשת שדות חסרים באדום
   - בדיקת תקינות הכמות

3. **ביצוע הייצור:**
   - קריאה לפרוצדורה produce_batch
...
![image](https://github.com/user-attachments/assets/30da1af4-6b4f-418a-a44e-a2695dbe5e9d)


1. **הפעלת הבדיקה:**
   - לחיצה על כפתור "בדוק חומרים למוצר"
   - קריאה לפרוצדורה get_materials_summary_for_product

2. **הצגת הסיכום:**
   - רשימת חומרים נדרשים למוצר
   - עבור כל חומר:
     - שם החומר
     - כמות נדרשת
     - כמות זמינה במלאי
     - מצב: OK (ירוק) או חסר (אדום)

#### טיפול במחסור בחומרי גלם

![image](https://github.com/user-attachments/assets/1777d899-02c2-48ab-8c88-cc18723d280a)

כאשר הייצור נכשל בגלל מחסור בחומרי גלם:

1. **זיהוי המחסור:**
   - פרוצדורת הייצור מזהה מחסור
   - הצגת הודעת אזהרה למשתמש
   - פתיחת ממשק השלמת מלאי אוטומטית

2. **ממשק השלמת מלאי:**
   - תפריט נפתח עם כל חומרי הגלם הזמינים
   - שדה קלט לכמות החומר
   - כפתור "➕ הוסף מלאי"

3. **הוספת חומר למלאי:**
   - בדיקה אם החומר קיים בסניף
   - עדכון כמות קיימת או הוספת רשומה חדשה
   - עדכון אוטומטי של טבלת המתכונים
   - הצגת הודעת אישור

### תכונות מתקדמות

#### אינטגרציה עם מסד הנתונים
- **פרוצדורות מאוחסנות:** שימוש ב-CALL statements
- **טרנזקציות:** שמירה על שלמות הנתונים
- **הודעות מהמסד:** האזנה ל-NOTICE messages


#### טבלאות מרכזיות
- **Branches:** סניפי המאפייה
- **Employee:** עובדים לפי סניפים
- **BakedGoods:** מוצרי המאפייה
- **RawMaterials:** חומרי גלם לפי סניפים
- **Recipe:** מתכונים ויחסי חומרים


---

## 📊 דוחות ושאילתות – SELECT, UPDATE, DELETE עם תמיכה ב־Transactions
מסך הדוחות והשאילתות מאפשר הרצת שאילתות מסוגים שונים (SELECT, UPDATE, DELETE), תוך שימוש במנגנון טרנזקציות לניהול שינויים בצורה בטוחה ומבוקרת.


#### 🧭 מבנה המסך

המסך מורכב משני חלקים עיקריים:

1. **סרגל צד לניהול והרצת השאילתות - Sidebar**
2. **תיבת תוצאות – הצגת פלטי השאילתות המורצות**


#### 🧩 סרגל צד לניהול והרצת השאילתות - Sidebar

##### 🔘 חלק ראשון – כפתורי שליטה

- **רענון** – מסתיר את תיבת התוצאות ומבצע איפוס למצב ההתחלתי.
- **אשר שינויים** – מבצע `COMMIT` לעסקה האחרונה שבוצעה (עדכון או מחיקה).
- **בטל שינויים** – מבצע `ROLLBACK` לעסקה האחרונה (שחזור למצב הקודם).

> ⚠️ הכפתורים "אשר שינויים" ו־"בטל שינויים" זמינים **רק לאחר שבוצעה פעולה שמשנה את הנתונים** (UPDATE / DELETE).  
> בעת ביצוע פעולה כזו, תוצג הודעה על מסך הממשק המודיעה שיש עסקה פעילה שניתן לאשר או לבטל.


##### 📄 חלק שני – דוחות (SELECT)

- כולל **8 שאילתות SELECT** מורכבות, שנלקחו משלב 2.
- כל שאילתה מריצה דו"ח שונה (כגון: מכירות, עובדים פעילים, סניפים לפי קטגוריות מוצרים ועוד).
- התוצאה מוצגת בתיבת התוצאות, יחד עם **כותרת מותאמת** לכל דו"ח.


##### ✍️ חלק שלישי – שאילתות עדכון (UPDATE)

- כולל **2 שאילתות עדכון**, גם הן משלב 2.
- לאחר ביצוע העדכון:
  - העסקה נשמרת במצב "ממתינה".
  - ניתן לאשר (`COMMIT`) או לבטל (`ROLLBACK`) את השינוי.
- עד שלא מתבצעת לחיצה על אחד הכפתורים – הנתונים לא משתנים בבסיס הנתונים בפועל.


##### ❌ חלק רביעי – שאילתות מחיקה (DELETE)

- כולל **3 שאילתות מחיקה** מובנות מראש.
- גם כאן, כל מחיקה נעטפת בטרנזקציה:
  - לאחר המחיקה תופיע הודעה על עסקה פעילה.
  - ניתן לבחור האם לאשר או לבטל.


#### 📥 חלון תוצאות

- משמש להצגת פלטי שאילתות SELECT בלבד.
- מוצג באופן דינמי – לאחר לחיצה על אחת מהשאילתות.
- לחיצה על כפתור **רענון** תסתיר את הפלט ותאפס את החלון.


💡 תכונות עיקריות:
- ✅ תמיכה ב־Transactions מלאות (COMMIT / ROLLBACK)
- 🔍 דוחות מוכנים מראש עם שאילתות מורכבות
- 🧠 ממשק אינטואיטיבי ונוח להרצת שאילתות מבלי לכתוב SQL ידנית
...
הפרויקט מורכב משני שלבים עיקריים:
1. **מסך ראשי** - נקודת כניסה מרכזית למערכת
2. **מודול ניהול עובדים** - מערכת CRUD מלאה לניהול נתוני עובדים
...
4. ודאו שכל הקבצים נמצאים באותה תיקייה:
   - `main_menu.py` - המסך הראשי
   - `employee_screen.py` - מסך ניהול עובדים
   - `baked_goods_screen.py` - מסך ניהול מוצרי מאפה
   - `proc_screen.py` - מסך ניהול פסי ייצור  
   - `reports_screen.py` - מסך דוחות ושאילתות
...
### תיאור המסך
המסך הראשי מהווה נקודת כניסה מרכזית המאפשרת ניווט נוח לכל מודולי המערכת.

### רכיבי המסך
1. **כותרת ברוכים הבאים** - הצגת שם המערכת בעיצוב נקי
2. **תפריט ניווט מרכזי** עם 4 כפתורים עיקריים:
   - **ניהול עובדים** 👥 - פתיחת מסך ניהול העובדים
   - **ניהול מוצרי מאפה** 🥖 - גישה למסך ניהול המוצרים  
   - **ניהול פסי ייצור** ⚙️ - מעקב תהליכי ייצור
   - **דוחות ושאילתות** 📊 - יצירת דוחות וניתוחים
3. **כפתור יציאה** - סגירת המערכת (עיצוב אדום בולט)

### פונקציונליות
- כל כפתור פותח את המודול הרלוונטי בחלון נפרד
- המסך הראשי נשאר פתוח לניווט נוסף
- טיפול בשגיאות עם הודעות ברורות בעברית
- עיצוב אינטואיטיבי עם כפתורים גדולים וברורים

![המסך הראשי עם כל הכפתורים מסודרים אנכית]

## שלב ב': מודול ניהול עובדים 👥

### כניסה למודול
מהמסך הראשי, לחיצה על כפתור "ניהול עובדים" פותחת את מסך ניהול העובדים המתקדם.

### תיאור המודול
מערכת CRUD מלאה לניהול נתוני עובדים במאפייה עם ממשק גרפי מתוחכם.
...
- בוחר תאריך לידה
- תפריטים נפתחים: בחירת סניף ותפקיד (נטען מבסיס הנתונים)
- כפתורי "שמור" ו"ביטול"

### פעולות CRUD מפורטות

#### הוספת עובד חדש (Create)
1. לחיצה על "הוסף עובד"
2. טופס עריכה נפתח בצד ימין
3. מילוי כל השדות הנדרשים
4. בחירת סניף ותפקיד מהתפריטים
5. לחיצה על "שמור" - העובד נוסף לבסיס הנתונים והטבלה מתעדכנת

#### צפייה וחיפוש עובדים (Read)
- הצגת כל העובדים בטבלה הראשית
- חיפוש מתקדם לפי קריטריונים שונים
- סינון תוצאות בזמן אמת
- הצגת מספר העובדים הנמצאים

#### עדכון פרטי עובד (Update)
1. בחירת עובד מהטבלה (לחיצה על השורה)
2. לחיצה על "עדכן עובד"
3. טופס עריכה נפתח עם הפרטים הקיימים
4. עריכת הפרטים הרצויים
5. שמירה - עדכון בסיס הנתונים וריענון הטבלה

#### מחיקת עובד (Delete)
1. בחירת עובד מהטבלה
2. לחיצה על "מחק עובד"
3. הצגת הודעת אישור עם פרטי העובד
4. אישור המחיקה - הסרה מבסיס הנתונים והטבלה

### צילומי מסך

#### המסך הראשי של ניהול העובדים
...
![image](https://github.com/user-attachments/assets/dc1295e5-19b1-441c-b27c-71a854043b9b)
...
![image](https://github.com/user-attachments/assets/1b87101a-aa23-4ae2-884c-268ce9ff51f2)
...
![image](https://github.com/user-attachments/assets/6115b820-908a-430f-8d66-85249d9758c5)
...
![image](https://github.com/user-attachments/assets/8edb2d1a-f2bf-47f8-a066-5344efd90ef0)
...
# שלב ד': מודול הרצת ייצור ⚙️

## כניסה למודול
מהמסך הראשי, לחיצה על כפתור "ניהול פסי ייצור" פותחת את מסך הרצת הייצור המתקדם.

## תיאור המודול
מערכת ניהול תהליכי ייצור חכמה הכוללת בדיקת מלאי, הרצת פרוצדורות ייצור, וניהול חומרי גלם בזמן אמת.

## ארכיטקטורת המערכת
המודול בנוי על בסיס ארכיטקטורת שכבות מפורדות עם אינטגרציה מלאה למסד הנתונים:

### 1. שכבת הממשק הגרפי (GUI Layer)
```python
class ProduceApp:
    - __init__(): אתחול החלון והטעינת נתונים מהמסד
    - create_combobox(): יצירת תפריטים נפתחים דינמיים
    - create_input(): יצירת שדות קלט מעוצבים
    - validate_and_submit(): ואליטציה וביצוע הייצור
```

### 2. שכבת הלוגיקה העסקית (Business Logic Layer)
```python
- produce_batch(): הרצת פרוצדורת ייצור עם טיפול בהודעות
- show_material_input(): הצגת ממשק השלמת מלאי
- insert_material(): הוספת חומרי גלם למלאי
- show_materials_summary(): הצגת סיכום חומרים נדרשים
```

### 3. שכבת מסד הנתונים (Database Layer)
```sql
- CALL produce_batch(%s, %s, %s, %s): פרוצדורה מאוחסנת לייצור
- get_materials_summary_for_product(): סיכום חומרים למוצר
- טבלאות: Branches, Employee, BakedGoods, RawMaterials, Recipe
```
...
### 3. כפתור בדיקת חומרים (שורה נפרדת)
...
- תווית: "כמות לייצור:"
- שדה קלט מספרי עם יישור לימין
- ואליטציה: בדיקת מספר חיובי
...
- טקסט: "✅ הרץ ייצור"
- עיצוב: ירוק עם גופן גדול
- מיקום: מרכזי בתחתית המסך
...
2. **ממשק השלמת מלאי:**
   - מסגרת כחולה עם כותרת "השלמת מלאי חסר"
   - תפריט נפתח עם כל חומרי הגלם הזמינים
   - שדה קלט לכמות החומר
   - כפתור "➕ הוסף מלאי"
...
### אבטחה ואמינות
- **טיפול בשגיאות:** כל פעולה עטופה ב-try-catch עם הודעות ברורות
- **ואליטציה מתקדמת:** בדיקת תקינות בכל שלב
- **חזרה לטעינה:** כפתור רענון לטעינה מחודשת
...
### חוויית משתמש משופרת
- **עיצוב מודרני:** צבעים וגופנים עכשוויים
- **ממשק רספונסיבי:** התאמה דינמית לתוכן
- **הדרכה ויזואלית:** אייקונים וצבעים משמעותיים
