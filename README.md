...
# מערכת ניהול מאפייה - מסך ראשי 🏠

מסך ראשי מרכזי למערכת ניהול מאפייה המאפשר ניווט לכל המודולים השונים במערכת.

## הוראות הפעלה

### התקנה
1. וודא שכל מודולי המערכת נמצאים באותה תיקייה:
...
* **PostgreSQL**: בסיס הנתונים המרכזי לכל המודולים
* **psycopg2**: חיבור בין Python לבסיס הנתונים

## תיאור המערכת

המסך הראשי מהווה נקודת כניסה מרכזית לכל מודולי המערכת. המסך כולל:

### 1. כותרת ברוכים הבאים
![כותרת המערכת]
* הצגת כותרת חברותית "ברוכים הבאים למערכת הניהול"
* עיצוב נקי ומקצועי

### 2. תפריט ניווט מרכזי
![כפתורי הניווט]
המסך מכיל 4 כפתורי ניווט עיקריים:

#### **ניהול עובדים** 👥
* פותח את מסך ניהול נתוני העובדים
* מאפשר הוספה, עריכה, מחיקה וחיפוש עובדים
* ניהול פרטים אישיים, תפקידים וסניפים

#### **ניהול מוצרי מאפה** 🥖
* גישה למסך ניהול המוצרים
* ניהול מלאי, מתכונים ותמחור
* מעקב אחר סוגי לחם ומאפים

#### **ניהול פסי ייצור** ⚙️
* פתיחת מסך ניהול הייצור
* תכנון ומעקב אחר תהליכי הייצור
* ניהול משמרות וקצב עבודה

#### **דוחות ושאילתות** 📊
* גישה למסך הדוחות והניתוחים
* יצירת דוחות מפורטים
* שאילתות מתקדמות על הנתונים

### 3. כפתור יציאה
![כפתור יציאה]
* כפתור אדום בולט לסגירת המערכת
* ממוקם בתחתית המסך

## פונקציונליות המערכת

### ניווט בין מסכים
* לחיצה על כל כפתור פותחת את המודול הרלוונטי
* כל מסך נפתח בחלון נפרד
* המסך הראשי נשאר פתוח לניווט נוסף

### טיפול בשגיאות
* המערכת מציגה הודעות שגיאה ברורות במקרה של בעיות
* בדיקת זמינות הקבצים לפני פתיחת מסכים
* הודעות שגיאה בעברית למשתמש הקצה

### עיצוב ונוחות השימוש
* ממשק פשוט ואינטואיטיבי
* כפתורים גדולים וברורים
* צבעים נעימים לעין (רקע אפור בהיר)
* טקסט בעברית לנוחות המשתמש

## מבנה הקבצים

המערכת מורכבת ממודולים נפרדים:
```
bakery_management/
├── main_menu.py          # המסך הראשי (קובץ זה)
├── employee_screen.py    # מסך ניהול עובדים  
├── baked_goods_screen.py # מסך ניהול מוצרי מאפה
├── proc_screen.py        # מסך ניהול פסי ייצור
└── reports_screen.py     # מסך דוחות ושאילתות
...
### ניווט למודולים
![דוגמה לפתיחת מסך ניהול עובדים מהמסך הראשי]

המסך הראשי מהווה שער כניסה נוח ומסודר לכל יכולות המערכת, ומאפשר למשתמשים לנווט בקלות בין המודולים השונים בהתאם לצרכיהם.

---

# מערכת ניהול עובדים - מאפייה 🍞
מערכת ממשק גרפי לניהול עובדים במאפייה המחוברת לבסיס נתונים PostgreSQL.
...
## תיאור המערכת
המערכת מספקת ממשק גרפי פשוט לניהול נתוני עובדים במאפייה. המסך מחולק לאזורים עיקריים:

### 1. אזור החיפוש (חלק עליון)
![איזור חיפוש]
* תפריט לבחירת קריטריון חיפוש (שם, טלפון, אימייל וכו')
* שדה להזנת טקסט החיפוש
* כפתורי "חפש" ו"נקה"

### 2. כפתורי פעולות
![כפתורי פעולות]
* **הוסף עובד**: פותח טופס להוספה
* **עדכן עובד**: עורך עובד נבחר
* **מחק עובד**: מוחק עובד נבחר
* **רענן**: טוען מחדש את הנתונים

### 3. טבלת העובדים (חלק שמאלי)
![טבלת עובדים]
* מציגה רשימת כל העובדים
* עמודות: שם, טלפון, אימייל, תאריך לידה, סניף, תפקיד
* ניתן לבחור עובד על ידי לחיצה על השורה

### 4. טופס עריכה (חלק ימני - מופיע בעת הצורך)
![טופס עריכה]
* שדות למילוי פרטי העובד
* תפריטים נפתחים לבחירת סניף ותפקיד
* כפתורי "שמור" ו"ביטול"
...
1. לחץ "הוסף עובד"
2. מלא את הפרטים בטופס שנפתח
3. לחץ "שמור"

### צפייה בעובדים (Read)
* כל העובדים מוצגים בטבלה הראשית
* השתמש בחיפוש למציאת עובד ספציפי
...
### הוספת עובד חדש
![טופס הוספת עובד עם כל השדות]

### חיפוש עובדים
![תוצאות חיפוש לפי שם עובד]

### עדכון פרטי עובד
![טופס עדכון עם פרטים קיימים]

## מבנה בסיס הנתונים
המערכת עובדת עם 3 טבלאות עיקריות:
* **Employee**: נתוני העובדים
* **Branches**: רשימת הסניפים
* **Roles**: רשימת התפקידים

המערכת מתחברת לבסיס הנתונים ומאפשרת ניהול מלא של המידע דרך הממשק הגרפי.
